#计分板
scoreboard objectives add 爆破-潜行判定 stat.sneakTime
...
scoreboard players set @a[tag=游戏中] 爆破-金钱 10
...
title @a[team=CT] subtitle [{"text":"§e§l击败全部对手或拆除炸弹以获得胜利"}]
title @a[team=T] subtitle [{"text":"§e§l击败全部对手或引爆炸弹以获得胜利"}]
...
execute @e[tag=VMW] ~ ~ ~ function vmwp:游戏/赏金爆破/地图设定
execute @e[tag=VMW] ~ ~ ~ function vmwp:游戏/赏金爆破/单局开始

scoreboard players tag @e[tag=VMW] add 全局游戏
scoreboard players tag @e[tag=VMW] add 开始2
