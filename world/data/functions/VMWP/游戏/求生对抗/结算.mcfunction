tellraw @a[tag=!PVE-游戏中] [{"text":"§b§l求生对抗前三名排行:\n"},{"text":"§a1st: "},{"selector":"@a[tag=1st]"},{"text":" §c分数: "},{"score":{"objective":"个人-第一击杀数","name":"@e[tag=VMW]"}}]
execute @p[tag=2nd] ~ ~ ~ tellraw @a[tag=!PVE-游戏中] [{"text":"§a2nd: "},{"selector":"@a[tag=2nd]"},{"text":" §c分数: "},{"score":{"objective":"个人-第二击杀数","name":"@e[tag=VMW]"}}]
