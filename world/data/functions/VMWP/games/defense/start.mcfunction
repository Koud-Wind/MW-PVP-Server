...
scoreboard objectives add CT-Death stat.deaths
scoreboard objectives setdisplay sidebar CT-lifeC
...
scoreboard players set @a[tag=InGame] money 20
...
title @a[team=CT] subtitle [{"text":"§eKill all opponents or stick to the target point to win"}]
title @a[team=T] subtitle [{"text":"§eKill all opponents or occupy target points to win"}]
...
execute @e[tag=VMW] ~ ~ ~ function vmwp:games/defense/setMap
execute @e[tag=VMW] ~ ~ ~ function vmwp:games/defense/startAGame

scoreboard players tag @e[tag=VMW] add GlobalGame
scoreboard players tag @e[tag=VMW] add start4
